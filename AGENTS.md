...
**Regra**: FAQ mostra 5 prioritárias (preço/payback live/OAuth/redes/garantia); payback abre por padrão; `#faq-savings-loss` sync com ROI; nudge de período cita perda.
**Status**: ativo
...
**Regra**: landing grava `blackink_calc_budget` (session+local) e passa `?budget=` nos plan-links; cadastro mostra bridge + subtitle personalizado com a perda est. (25%).
**Status**: ativo
